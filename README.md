...
- `:app` 모듈은 `com.android.library` 플러그인을 사용하는 **Android Library 모듈**입니다.
- 따라서 `:app`에는 `applicationId`를 두지 않으며, 외부 앱이나 샘플 앱이 의존성으로 가져가서 사용합니다.
- 실행 가능한 앱이 필요하면 별도 호스트 앱 모듈인 `:lumosapp`에서 `implementation(project(":app"))` 형태로 라이브러리를 참조합니다.
- 라이브러리 코어 검증은 `:app:testDebugUnitTest`로 수행하고, 실제 카메라/MediaPipe 연동은 호스트 앱 또는 샘플 앱에서 검증합니다.
